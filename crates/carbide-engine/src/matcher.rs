...
use chrono::Utc;
use carbide_core::{AssetSymbol, CarbideError, CarbideResult, OrderId};
...
impl MatchingEngine {
    ...
    pub fn submit_order(&self, create: CreateOrder) -> CarbideResult<(Order, Vec<TradeExecution>)> {
        if create.quantity <= Decimal::ZERO {
            return Err(CarbideError::InvalidInput("Quantity must be positive".into()));
        }
        if create.price < Decimal::ZERO {
            return Err(CarbideError::InvalidInput("Price must be non-negative".into()));
        }
    ...
    pub fn cancel_order(
        ...
        let book = self
            .books
            .get(&asset)
            .ok_or_else(|| CarbideError::not_found("OrderBook", asset))?;

        book.cancel(order_id, side)
            .ok_or_else(|| CarbideError::not_found("Order", order_id))
    }
