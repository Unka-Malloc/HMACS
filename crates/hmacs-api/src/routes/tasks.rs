...
use hmacs_core::TaskId;
use hmacs_task::{CreateBid, CreateTask, DeliverTask};
use uuid::Uuid;
...
) -> ApiResult<Json<hmacs_task::Task>> {
    let task = state
        .task_service
...
async fn list_tasks(
    State(state): State<AppState>,
) -> ApiResult<Json<Vec<hmacs_task::Task>>> {
...
async fn list_bids(
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> ApiResult<Json<Vec<hmacs_task::Bid>>> {
