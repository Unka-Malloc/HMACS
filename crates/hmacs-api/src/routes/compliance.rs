...
async fn get_jurisdiction_rules(
    ...
    let jurisdiction = match code.to_uppercase().as_str() {
        ...
        "AE" => Jurisdiction::Ae,
        _ => {
            return Err(ApiError(hmacs_core::HmacsError::InvalidInput(format!(
    ...
    let rules = hmacs_compliance::JurisdictionRules::for_jurisdiction(jurisdiction);
    let json = serde_json::to_value(rules)
        .map_err(|e| ApiError(hmacs_core::HmacsError::Internal(e.to_string())))?;
    Ok(Json(json))
}
