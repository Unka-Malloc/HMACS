...
use hmacs_core::{
    AgentPermission, DelegatedKey, DelegatedKeyId, HmacsError, HmacsResult, ParticipantId,
    ParticipantKind,
};
...
impl DelegationService {
    ...
    pub fn get_key(&self, key_id: DelegatedKeyId) -> HmacsResult<DelegatedKey> {
        self.keys
            .read()
            .get(&key_id)
            .cloned()
            .ok_or_else(|| HmacsError::not_found("DelegatedKey", key_id))
    }

    pub fn revoke_key(&self, key_id: DelegatedKeyId, master_id: ParticipantId) -> HmacsResult<()> {
    ...
    pub fn check_agent_permission(
        ...
        let key_ids = agent_keys
            .get(&agent_id)
            .ok_or_else(|| HmacsError::Unauthorized("No delegated keys for this agent".into()))?;
        ...
        Err(HmacsError::Forbidden(format!(
