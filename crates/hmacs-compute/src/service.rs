...
use chrono::Utc;
use hmacs_core::{
    AssetSymbol, ComputeLeaseId, ComputeResourceId, HmacsError, HmacsResult, ParticipantId,
};
...
impl ComputeService {
    ...
    pub fn get_resource(&self, id: ComputeResourceId) -> HmacsResult<ComputeResource> {
        self.resources
            .read()
            .get(&id)
            .cloned()
            .ok_or_else(|| HmacsError::not_found("ComputeResource", id))
    }
    ...
    ) -> HmacsResult<ComputeLease> {
        let listings = self.listings.read();
        let listing = listings
            .get(&listing_id)
            .ok_or_else(|| HmacsError::not_found("ComputeListing", listing_id))?;

        if !listing.active {
            return Err(HmacsError::InvalidInput("Listing is not active".into()));
        }
    ...
    pub fn report_usage(
        ...
        let lease = leases
            .get_mut(&lease_id)
            .ok_or_else(|| HmacsError::not_found("ComputeLease", lease_id))?;
        ...
        if lease.status != LeaseStatus::Active {
            return Err(HmacsError::InvalidInput("Lease is not active".into()));
        }
    ...
    pub fn get_lease(&self, lease_id: ComputeLeaseId) -> HmacsResult<ComputeLease> {
        self.leases
            .read()
            .get(&lease_id)
            .cloned()
