...
use carbide_core::{CarbideError, ParticipantId};
use carbide_identity::{verify_token, Claims, JwtConfig};
use std::sync::Arc;
...
pub async fn auth_middleware(
    ...
    let auth_header = request
        ...
        .ok_or_else(|| {
            ApiError(CarbideError::Unauthorized(
    ...
    let participant_id: uuid::Uuid = claims
        .participant_id
        .parse()
        .map_err(|_| ApiError(CarbideError::Unauthorized("Invalid participant ID in token".into())))?;
