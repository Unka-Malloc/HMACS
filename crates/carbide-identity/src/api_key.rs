...
use carbide_core::{ApiKeyId, CarbideError, ParticipantId};
use rand::Rng;
use serde::{Deserialize, Serialize};
use sha2::{Digest, Sha256};

const API_KEY_PREFIX: &str = "carbide_";
...
pub fn verify_api_key(raw_key: &str, stored_hash: &str) -> Result<(), CarbideError> {
    let computed_hash = hash_api_key(raw_key);
    if computed_hash == stored_hash {
        Ok(())
    } else {
        Err(CarbideError::Unauthorized("Invalid API key".to_string()))
    }
}
...
mod tests {
    ...
    fn test_key_generation() {
        ...
        assert!(key.starts_with("carbide_"));
