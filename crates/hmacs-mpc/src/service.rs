...
use chrono::Utc;
use hmacs_core::{AssetSymbol, HmacsError, HmacsResult, MpcTaskId, ParticipantId};
...
impl MpcService {
    ...
    pub fn join_race(
        ...
        if self.is_blacklisted(agent_id) {
            return Err(HmacsError::AgentBlacklisted(format!(
        ...
        let task = tasks
            .get_mut(&task_id)
            .ok_or_else(|| HmacsError::not_found("MpcTask", task_id))?;

        if task.state != MpcTaskState::Open && task.state != MpcTaskState::Racing {
            return Err(HmacsError::InvalidStateTransition {
        ...
        if task.participants.len() >= task.total_agents {
            return Err(HmacsError::InvalidInput("Race is full".into()));
        }
    ...
    pub fn start_race(&self, task_id: MpcTaskId) -> HmacsResult<(ShareSender, MpcTaskId)> {
        let mut tasks = self.tasks.write();
        let task = tasks
            .get_mut(&task_id)
            .ok_or_else(|| HmacsError::not_found("MpcTask", task_id))?;
    ...
    pub fn fail_and_retry(&self, task_id: MpcTaskId) -> HmacsResult<Option<MpcTaskId>> {
        let mut tasks = self.tasks.write();
        let task = tasks
            .get_mut(&task_id)
            .ok_or_else(|| HmacsError::not_found("MpcTask", task_id))?;
    ...
    pub fn get_task(&self, task_id: MpcTaskId) -> HmacsResult<MpcTask> {
        self.tasks
            .read()
            .get(&task_id)
            .cloned()
