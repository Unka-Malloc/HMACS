...
use chrono::Utc;
use carbide_core::{BidId, CarbideError, CarbideResult, ParticipantId, TaskId};
...
impl TaskService {
    ...
    pub fn get_task(&self, task_id: TaskId) -> CarbideResult<Task> {
        self.tasks
            .read()
            .get(&task_id)
            .cloned()
            .ok_or_else(|| CarbideError::not_found("Task", task_id))
    }
    ...
    pub fn place_bid(
        ...
        let task = tasks
            ...
            .ok_or_else(|| CarbideError::not_found("Task", input.task_id))?;
    ...
    pub fn accept_bid(
        ...
        // Validate the bid exists and belongs to this task
        {
            let bid = bids
                .get(&bid_id)
                .ok_or_else(|| CarbideError::not_found("Bid", bid_id))?;
            if bid.task_id != task_id {
                return Err(CarbideError::InvalidInput("Bid does not belong to this task".into()));
            }
...
mod tests {
    use super::*;
    use carbide_core::{AssetSymbol, ParticipantRestriction};
