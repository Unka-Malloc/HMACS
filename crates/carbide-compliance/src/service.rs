...
use carbide_core::{
    AssetSymbol, ConsentRecordId, CarbideError, CarbideResult, KycRecordId, ParticipantId,
    TravelRuleMessageId,
};
...
impl ComplianceService {
    ...
    pub fn check_sanctions_clearance(
        ...
        match latest {
            ...
            Some(s) if s.is_blocked() => Err(CarbideError::SanctionsMatch(format!(
                "Participant {} has a confirmed sanctions match",
                participant_id
            ))),
            Some(_) => Err(CarbideError::ComplianceBlocked {
                reason: "Sanctions screening pending review".into(),
                jurisdiction: jurisdiction.to_string(),
            }),
            None => Err(CarbideError::ComplianceBlocked {
    ...
    ) -> CarbideResult<Vec<ComplianceAlert>> {
    ...
    pub fn check_travel_rule(
    ...
    ) -> CarbideResult<Option<TravelRuleMessage>> {
        ...
        travel_rule::validate_originator_info(originator, jurisdiction)
            .map_err(CarbideError::TravelRuleViolation)?;
    ...
    pub fn pre_transaction_check(
        ...
        if !assessment.is_acceptable() {
            return Err(CarbideError::ComplianceBlocked {
                reason: format!(
    ...
    fn get_rules(&self, jurisdiction: Jurisdiction) -> CarbideResult<JurisdictionRules> {
        self.registry
            .get(&jurisdiction)
            .cloned()
            .ok_or_else(|| CarbideError::InvalidInput(format!("Unknown jurisdiction: {}", jurisdiction)))
    }
}
...
impl Default for ComplianceService {
    fn default() -> Self {
        Self::new(VaspInfo {
            name: "Carbide Platform".into(),
            lei: None,
            jurisdiction: Jurisdiction::Sg,
            registration_number: None,
            website: Some("https://carbide.io".into()),
            address: None,
        })
    }
}
...
mod tests {
    ...
    use carbide_core::TransactionId;
